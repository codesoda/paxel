# Builds prompts for V3 5-axis scoring.
# 5-axis anchors: Execution Leverage, Steering, Engineering Quality,
# Product Thinking, Planning.
# (Phase 2 Priority 4: the axis formerly called Throughput was renamed to
# Execution Leverage and redefined from raw velocity to outcome-per-input
# — subagent dispatches that ship and plan-then-ship patterns.)
# CRITICAL_FRAMING below intentionally DIVERGES from the narrative-path framing:
# the "velocity is evidence of effective engineering" line was removed here because
# it contradicts the Execution Leverage anchors (score outcome, not raw volume).
# Do NOT re-add it — the narrative path keeps the old line on its separate route.
#
# CALIBRATION (2026-06-04): the episode LLM previously clustered competent
# episodes at 7-8 and rarely used 9-10, compressing the overall upload score
# range. The CALIBRATION_GUIDANCE section below tells the scorer to use the FULL
# 1-10 scale (7 = median operator, 9-10 attainable for top-decile work, both
# tails real) WITHOUT re-centering — 7 stays the median so the change widens
# spread rather than lowering everyone. Pairs with
# EpisodeSummarizer::PROMPT_VERSION v15.
class V3ScoringPromptBuilder
  CRITICAL_FRAMING = <<~FRAMING.freeze
    IMPORTANT FRAMING:
  ...
  # Calibration guidance: force use of the full 1-10 scale. The anchors above
  # describe each quality level well, but the scorer's DEFAULT behavior is to
  # park competent work at 7-8 and withhold 9-10 — which compresses every
  # upload into a narrow band. This section attacks that default directly.
  # Design intent: keep 7 as the median (do NOT re-center downward — that would
  # just lower everyone), and widen the tails so genuinely strong episodes reach
  # 8-10 and genuinely weak ones reach 3-5.
  CALIBRATION_GUIDANCE = <<~CAL.freeze
  ...
  AXIS_ANCHORS = {
    "execution_leverage" => {
      label: "Execution Leverage (EL): Does their effort close outcome loops?",
      anchors: [
    ...
    "steering" => {
      label: "Steering (S): Do they control the AI?",
      anchors: [
    ...
    "engineering_quality" => {
      label: "Engineering Quality (EQ): Are they a good engineer?",
      anchors: [
    ...
    "product_thinking" => {
      label: "Product Thinking (PT): Do they understand users?",
      anchors: [
    ...
    "planning" => {
      label: "Planning (P): Do they think before they build?",
      anchors: [
  ...
  def self.build_episode_prompt(episode_data, velocity_context: nil, language_band: nil)
    anchors_text = AXIS_ANCHORS.map do |axis, info|
      "### #{info[:label]}\n#{info[:anchors].join("\n")}"
    end.join("\n\n")

    velocity_section = velocity_context ? "\n\nVelocity context:\n#{velocity_context}" : ""
    band_section = if language_band
      "\n\nLanguage band: #{language_band['band']}. " \
      "Allowed vocabulary: #{language_band['vocabulary'].join(', ')}. " \
      "Banned words: #{language_band['banned'].join(', ')}."
    else
      ""
    end
